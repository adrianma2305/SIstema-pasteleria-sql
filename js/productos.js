...
function agregarIngredienteATemporal() { 
    ...
    if(!select.value || !cantInput.value || parseFloat(cantInput.value) <= 0) return mostrarNotificacion('Atención', 'Selecciona un ingrediente y digita una cantidad mayor a 0.', 'warning'); 
    ...
    recetaTemporal.push({ insumo_id: insumoId, nombre: insumoObj.nombre, cantidad_lote_visible: cantidadDigitada, cantidad_necesaria: cantidadUnitariaCalculada, subtotal: subtotalCostoUnitario }); 
    cantInput.value = ""; 
...
async function agregarProducto(event) { 
    ...
    if (!nombre || isNaN(precio) || precio <= 0 || !categoria_id) return mostrarNotificacion("Atención", "Revisa que el precio sea mayor a cero y no falten datos.", "warning"); 
    
    try { 
        const respuesta = await fetch(`${API_URL}/productos`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nombre, precio, categoria_id, receta: recetaTemporal }) }); 
        if (!respuesta.ok) throw new Error("Error"); 
        
        mostrarNotificacion("¡Guardado!", "Producto y receta respaldados en la nube.", "success"); 
        document.getElementById("form-agregar").reset(); 
        limpiarRecetaTemporal(); 
        bootstrap.Modal.getInstance(document.getElementById("modalAgregar")).hide(); 
        cargarProductos(); 
    } catch (error) { mostrarNotificacion("Error", "Fallo al comunicar con Azure.", "error"); } 
}
...
async function ejecutarProduccion(event) { 
    ...
    if(!producto_id || isNaN(cantidad_producida) || cantidad_producida <= 0) return mostrarNotificacion("Atención", "Ingresa una cantidad mayor a 0", "warning"); 
    
    try { 
        const respuesta = await fetch(`${API_URL}/produccion`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ producto_id, cantidad_producida, usuario_id }) }); 
        if(!respuesta.ok) throw new Error(await respuesta.text()); 
        
        mostrarNotificacion("¡Horneado Exitoso!", "Se descontaron insumos y subió la vitrina.", "success"); 
        document.getElementById("form-produccion").reset(); 
        bootstrap.Modal.getInstance(document.getElementById("modalProduccion")).hide(); 
        cargarProductos(); 
    } catch (error) { mostrarNotificacion("Fallo en Producción", error.message, "error"); } 
}
